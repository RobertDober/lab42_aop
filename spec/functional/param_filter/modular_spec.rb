...
describe AOP do
  context 'param filter aspects with module concerns' do 
    let( :mod1 ){ aop_module{
      def a1 a; a.succ end
      def b1 b; 2*b end
    } }
    ...
    context 'using the simplest module as concern' do 
      before do
        class_under_test.param_filter mod1 do |*args|
          args.first.succ
        end
      end
    ...
    context 'using the complex module as concern' do 
      before do
        class_under_test.param_filter mod2 do |*args|
          args.first.succ
        end
      end
      it 'filters the module\'s method' do
        expect( subject.b3 0 ).to eq 3
      end
    ...
    context 'using the complex module as concern (with include included)' do 
      ...
      context 'method' do 
        before do
          ...
          class_under_test.param_filter mod2, :filter, include_included: true
        end
        it_behaves_like 'including included'
      end # context 'method'
      context 'block' do 
        before do
          class_under_test.param_filter mod2, include_included: true do |*a|
            a.first.succ
          end
        end
        it_behaves_like 'including included'
  ...
  end # context 'param filter aspects with module concerns'
end # describe AOP
