...
describe AOP do
  context 'before aspects with module concerns' do
    let( :mod1 ){ aop_module{
      def a1; end
      def b1; end
      private
    ...
    context 'using the simplest module as a concern' do
      before do
        class_under_test.before mod1 do
          @c += 1
        end
      end
    ...
    context 'using a complex (including itself a module) as a concern' do
      before do
        class_under_test.new_method :before_mod3 do
          @c += 1
        end
      end
      ...
      context 'block aspect with default behavior' do
        before do
          class_under_test.before mod3 do
            @c += 1
          end
        end
      ...
      context 'block aspect with include_include behavior' do
        before do
          class_under_test.before mod3, include_included: true do
            @c += 1
          end
        end
      ...
      context 'method aspect with default behavior' do
        before do
          class_under_test.before mod3, :before_mod3
        end
        it_behaves_like 'default'
      ...
      context 'method aspect with include_include behavior' do
        before do
          class_under_test.before mod3, :before_mod3, include_included: true
        end
        it_behaves_like 'including included'
  ...
  end # context 'before aspects with module concerns'
end # describe AOP
