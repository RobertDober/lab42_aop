...
describe AOP do
  ...
  context 'cross concern (using before block)' do 
    before do
      class_under_test.before %r{\Aa} do @c += 1 end
    end
  ...
  end # context 'cross concern (using before)'

  context 'cross concern (using before method)' do 
    before do
      class_under_test.new_method :bef_b do @c+= 1 end
      class_under_test.before %r{\Ab\d}, :bef_b
    end
  ...
  end # context 'cross concern (using before method)'

  context 'cross concern (mixing before blocks and methods)' do 
    before do
      class_under_test.before %r{\Aa\d} do @c += 1 end
      
      class_under_test.new_method :tripple_c do @c *= 3 end
      class_under_test.before %r{\A.1}, :tripple_c
    end

    context 'a1 triggers both, a2 triggers increment, b1 triggers trippling' do 
      it{ subject.a1; expect( subject.c ).to eq 4 }
  ...
  end # context 'cross concern (mixing before blocks and methods)'
end # describe AOP
